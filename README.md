# lb-sourdough
A starter for an API, with a single table backend. An API should be generally useful, adequately secure, and relatively easy to use.  

# Expectations
* The system is enabled by a JWT token called a starter-token
* The system expects to be run over an encrypted connection
* A starter-token is manually created at https://jwt.io or it can be generated by an application

## Strategy
* Single database, single table design
* Use postgres __schemata__ for versioning the API
* Separate API functions into separate __sql files__  
* Use __docker-compose__ to pull it all together
* Provide a working __example__

## Patterns
* Get token, use token to access API function(s), perform API function(s)
* The starter-token begets a woden-token, the woden-token begets an owner-token, the owner-token begets an application-token, and so it goes.
 

# Example: Woden API
The Germanic chief god, distributor of talents, wisdom and war. In this case, Woden distributes access and permissions via JSON Web Tokens. The origin, the begining, the start. Woden is the perfect name for this example API.

In this example, the administrator of administrators is called __Woden__ or the woden.  Woden gets authority from the starter token of starter tokens known as the __first-token__... very autopoetic. 

Woden creates and tracks of all the applications that make up a system. Including each application's starter-token. Beside, Woden the Woden API has users. A Woden API user is called an __owner__, more specifically an application owner. The woden can create owners and delegate authority to those owners. In otherwords, owner's can create applications and application starter-tokens.  And Woden said let there be development.   

## The First-Token
The first-token authorizes access to the signin function of the Woden API.  
The first-token is created manually using AuthO's token application at https://jwt.io 
* payload: {"role": "guest_wdn"}
* jwt.io requires a password and that password should be at least 32 characters long and is the same value as POSTGRES_JWT_SECRET

## Important
* Change your starter-token. The starter-token shown below is dependent on the POSTGRES_JWT_SECRET.  Changing the POSTGRES_JWT_SECRET will change the starter-token.  The starter-token token below will work for every application that uses the "PASSWORDmustBEATLEAST32CHARSLONGLONG" secret.  Not good. Change your password.

## Woden API 1.1.0 (wdn_1_1_0)
New feature:
* Delegate the woden's authority to application owners.  

Functions
* app(form JSON) - insert an application, generates an application specific JWT
* owner(form JSON) - insert an owner by woden, encrypts password
* siginin(form JSON) - insert an attempt to login, returns JWT on success

### Environment Setup (.env)
.env file goes in folder with docker-compose.yml
```
    # Postgres
    POSTGRES_DB=wdn_db
    POSTGRES_USER=postgres
    POSTGRES_PASSWORD=mysecretdatabasepassword
    POSTGRES_JWT_SECRET=PASSWORDmustBEATLEAST32CHARSLONGLONG
    # PostREST
    PGRST_DB_SCHEMA=wdn_schema_1_0_0,wdn_schema_1_1_0
    # PGRST_DB_SCHEMA=wdn_schema_1_0_0 
    PGRST_DB_ANON_ROLE=guest_wdn
    # Lyttlebit
    LB_STARTER={"name":"woden@lyttlebit.com","password":"a1A!aaaa"}
    LB_GUEST_PASSWORD=mysecretclientpassword
```
* Added the new version wdn_schema_1_1_0 to PGRST_DB_SCHEMA 
* Woden API clients have access to all versions: wdn_schema_1_0_0 and wdn_schema_1_1_0

### Simple Client: Curl
```
    # First Token (JWT)
        export FIRST_TOKEN=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJMeXR0bGVCaXQiLCJzdWIiOiJPcmlnaW4iLCJuYW1lIjoiV29kZW4iLCJyb2xlIjoiZ3Vlc3Rfd2duIn0.XjYxFfJ4HvgP6T7OupQdeMuxA9_WZCzRYRUGuVhNUQ4

    # Signin as the woden and get the woden's token note: Content-Profile directs API calls to version 1.1.0
        export WODENS_TOKEN=$(curl http://localhost:3100/rpc/signin -X POST \
                -H "Authorization: Bearer $FIRST_TOKEN" \
                -H "Content-Type: application/json" \
                -H "Content-Profile: wdn_schema_1_1_0" \
                -H "Prefer: params=single-object" \
                -d '{"name":"woden@lyttlebit.com", "password":"a1A!aaaa"}')

    # pull data out of json and set a one time token for the woden 
        export WODENS_TOKEN=$(echo $WODENS_TOKEN | grep -o '["^][a-zA-Z0-9_\-]*[\.][a-zA-Z0-9_\-]*[\.][a-zA-Z0-9_\-]*["$]')
    
    # get rid of enclosing quotes
        export WODENS_TOKEN=$(echo $WODENS_TOKEN | sed 's/"//g' )

    # use the wodens token to add an application
        curl http://localhost:3100/rpc/app -X POST \
             -H "Authorization: Bearer $WODENS_TOKEN"   \
             -H "Content-Type: application/json" \
             -H "Content-Profile: wdn_schema_1_1_0" \
             -H "Prefer: params=single-object"\
             -d '{"name":"woden@1.1.0", "owner_id":"woden@lyttlebit.com"}'
             
# use woden's token to create an application owner with delegated authority to create applications

# use owner's token to add an application 


```
    
## Woden API 1.0.0 (wdn_1_0_0) deprecated

* app(JSON)- insert an application, generates an application specific JWT
* signin(JSON) - insert attempt to login, returns JWT on sucess





# Underview

# Woden API Underview
* Keep a list of applications
* Keep a list of application owners
* Provide an app-token for a specific application

## Owner 
 ```
DATABASE:   wdn_db
SCHEMA:     wdn_schema
TABLE:      Register
COLUMNS:    id, type, form, active, created, updated
INDEX:      id
ROLE:       guest_wgn
ROLE:       owner_editor
```
```
API: Owner, CRUD
    owner: {id, type, |email|, name, [password]}

               function           token           VERB     return   guest_wgn
               --------           -----           ----     ------   ---------
    * insert , owner(owner::JSON),<woden-token> , POST   , <status> E
    * select , owner(TEXT)      , <app-token>   , GET    , <owner>  N
    * select , owner(TEXT, JSON), <app-token>   , GET    , <owner>  N
    * update , owner(JSON)      , <owner-token> , POST   , <status> N
    * delete , owner(????,????) , <owner-token> , DELETE , <status> N

Permissions CRUDE: Create Read Update Delete Execute
                    guest_wgn   owner_editor
register                C           RUD
C owner(JSON)           E
R owner(TEXT)                       E
U owner(TEXT, JSON)                 E
D owner(????)                       E
C app(JSON)                         E
R app(TEXT)                         E
U app(TEXT, JSON)                   E
D app(????,????)                    E
Rules:
    * Anyone can add an owner
    * Ownership is designated by the id in the owner record
    * User can only modifiy (UD) their own data, row specific
    * User can see all applications in the portfolio
    * Application is owned by the person who enters it
    *

    <status> is {"status":"", "msg":""}
    <owner> is {"status":"200", "token":"<app-token>"}
    <woden-token> is {"iss": "LyttleBit", "sub": "Origin", "name": "Woden", "role": "wdn_guest", "type": "owner"}
    <app-token>   is {"iss": "LyttleBit", "sub": "Origin", "name": "Woden", "role": "????_guest", "type": "owner"}
```
## Signin
```
API: Signin, C
    signin:  {"type":"signin", "name":"<email>", "password":"<TEXT>"}

    * insert , signin(signin::JSON), <woden-token> , POST   , <owner>

    Dependencis
    owner:
    <owner-token> is

```
## App
```
API: App, form: {id, type, |name|, owner_id, token}
               function           token           VERB     return
               --------           -----           ----     ------
    * insert , app(JSON)        , <owner-token> , POST   , <status>
    * select , app(TEXT)        , <owner-token> , GET    , <status>
    * select , app(TEXT, JSON)  , <????-token>  , GET    , <status>
    * update , app(JSON)        , <????-token>  , POST   , <status>
    * delete , app(????,????)   , <????-token>  , DELETE , <status>


* [ ] is encrypted value
* | | is id value or part of a compound id value
* ( ) is parameter list
* { } is json attribute list
* < > is value/variable
* id attribute triggers an update
* ???? is a marker for undefined
* table has index on id



  roles
    guest_wgn
    owner_editor
 ```      
  tokens
        woden: claims: (iss, sub, name, role, type)
          type: app
          role: api_guest
        app-token: claims: (iss, sub, name, role, type)  
          _iss: LyttleBit_
          _sub: _
          _name: <application-name>_
          _roles:[guest_wgn]_
          _type: actor_
        _actor-token: claims: (iss, sub, name, role, type)_
          _iss:_
          _sub:_
          _name: <username>_
          _roles: []_
          _type:_

                    guest_wgn,  owner_editor
        register    C           

        app(JSON)               
        app(JSON)   

# Exmpl Model Overview

application_db
    api_schema
        woden ()
        register columns: (id, type, form, password, active, created, updated)
            app: atts: (id, type, app-name, version, username, [password], token)
              insert app (Authorization: woden)(creates:app-token)
              select app (Authorization: woden)(returns:app-token)
            actor: atts: (id, type, app_id, name,[password])
              insert actor (Authorization: app-token)
              select actor (app-token)(returns: actor-token)
              update actor (actor-token)
              delete/deactivate actor (actor-token)
        adoption columns: (id, type, form, active, created, updated)
            adoption
              insert adoption (actor-token)
              update adoption (actor-token)
              delete adoption (actor-token)
apis
    app(JSON)
    app(TEXT)
    actor(JSON)
    actor(TEXT)
roles
    api_guest
    guest_wgn
    actor_editor
tokens
      woden: claims: (iss, sub, name, role, type)
        type: app
        role: api_guest
      app-token: claims: (iss, sub, name, role, type)  
        _iss: LyttleBit_
        _sub: _
        _name: <application-name>_
        _roles:[guest_wgn]_
        _type: actor_
      _actor-token: claims: (iss, sub, name, role, type)_
        _iss:_
        _sub:_
        _name: <username>_
        _roles: []_
        _type:_

# History
* actor: upsert for actor
* roles: change from single role to multiple roles   
* woden: add iss, sub, name, roles type to woden  
* add insert test for app
* add select test for app  
