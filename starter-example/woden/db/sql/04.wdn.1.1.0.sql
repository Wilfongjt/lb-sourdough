...
insert into wdn_schema_1_1_0.register (reg_type, reg_form) values ('woden', (:lb_woden::JSONB || '{"type":"woden", "roles":"admin"}'::JSONB) );
    ...
    CREATE OR REPLACE FUNCTION wdn_schema_1_1_0.owner(form JSON)
    RETURNS JSONB AS $$
      Declare rc jsonb;
      Declare _model_owner JSONB;
      Declare _form JSONB;
      ...
      BEGIN
        ...
        EXCEPTION
          WHEN others then
            -- PERFORM wdn_schema_1_1_0.process_logger(format('{"status":"500", "msg":"Unknown ", "SQLSTATE":"%s", "role":"%s"}',SQLSTATE, _jwt_role)::JSONB);
        ...
        if _form ? 'id' then
          ...
          EXCEPTION
              WHEN unique_violation THEN
                  -- PERFORM wdn_schema_1_1_0.process_logger(_form || '{"status":"400", "msg":"Bad Request, duplicate owner"}'::JSONB);
      ...
      END;
    $$ LANGUAGE plpgsql;
    grant EXECUTE on FUNCTION wdn_schema_1_1_0.owner(JSON) to guest_wgn; -- upsert
    grant EXECUTE on FUNCTION wdn_schema_1_1_0.owner(JSON) to editor_wdn; -- upsert
    ...
    -- Permissions: EXECUTE
    -- Returns: JSONB
    CREATE OR REPLACE FUNCTION wdn_schema_1_1_0.owner_validate(form JSONB)
    RETURNS JSONB
    AS $$
    ...
    grant EXECUTE on FUNCTION wdn_schema_1_1_0.owner_validate(JSONB) to guest_wgn; -- upsert
    grant EXECUTE on FUNCTION wdn_schema_1_1_0.owner_validate(JSONB) to editor_wdn; -- upsert
    ...
    CREATE OR REPLACE FUNCTION wdn_schema_1_1_0.owner(id TEXT) RETURNS JSONB
    AS $$
      Select reg_form-'password' as owner from wdn_schema_1_1_0.register where reg_id=id and reg_type='owner';
    $$ LANGUAGE sql;
